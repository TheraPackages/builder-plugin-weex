...
export default {
  ...
  projectFileChanged(changes) {
    ...
    if (changes.event === 'change' && changes.filePath) {
      console.log('Received file changed notification:', changes)

      if (Path.basename(changes.filePath) === 'launch.json') {
        // launch.json
        this.processLaunchFileChange(changes.filePath);

      } else if (/\.vue$/.test(changes.filePath) || /\.we$/.test(changes.filePath)) {
        // *.vue  *.we  recompile from the main entry.
        this.buildService.build(undefined);

      } else if (/\.js$/.test(changes.filePath)) {
        // *.js
        this.processJsFileChange(changes.filePath);
      }
    } else if (changes.event === 'add') {

      if (/\.js$/.test(changes.filePath)) {
        console.log('Received file add notification:', changes)
        this.processJsFileChange(changes.filePath);

      } else if (Path.basename(changes.filePath) === 'launch.json') {
        // Do build when firstly start project.
        console.log('Received add launch.json notification:', changes)
        this.processLaunchFileChange(changes.filePath);
      }

    } else if (changes.event === 'addDir') {
      // Ignore add dir
    } else if (changes.event === 'unlink') {
      // Ignore delete file
    } else if (changes.event === 'unlinkDir') {
      // Ignore delete dir
    }
  },

  processLaunchFileChange(filePath) {
    var self = this;  // launch.json
    this.buildService.updateProjectConfig(function() {
      self.buildService.build(undefined)
    });
  },
  ...
  processJsFileChange(filePath) {
    ...
    function pushIfBundleJs(err, configs) {
      ...
      if (Path.dirname(filePath) === targetPath) {          // In build output path.
        ...
        this.buildService.build(undefined);                   // Rebuild when modify some files like tool.js
      }
    }
  },
  ...
  publishBuildResult(sourceFile, filePath) {
    ...
    if (filePath) {
      ...
      Fs.readFile(filePath, function(err, content) {
        ...
        BuilderNotifier.pushBuildResult({
          type: 'weex',
          name: basename + '.js',
          bundleUrl: filePath,
