...
export default class BuilderServiceWebpack extends BuilderService {
  ...
  build(config, source, outputDir) {
    ...
    var targetPath = outputDir ? outputDir : Path.join(Path.parse(source).dir, 'out');
  ...
  watchImpl(config, source, outputDir) {
    if (!source) {
      console.error('Watch source should not be empty!')
      return;
    }
    if (this.watcher && !this.watcher.isClosed()) {
      this.watcher.close()
    }
    this.watcher = Watch(source, {}, (evt, name) => {
      // evt = remove/update
