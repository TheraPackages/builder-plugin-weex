...
export default class BuilderServiceWebpack extends BuilderService {
  ...
  buildDebug(config, source, output) {
    super.buildDebug(config, source, output);
    return this.build(config, source, output);
  }

  buildRelease(config, source, output) {
    super.buildRelease(config, source, output);
    return this.build(config, source, output);
  }
  ...
  transform(source, ouputDir) {
    if (!source) {
      return new Promise(function(resolve, reject){reject('Input source file should not be empty!')});
    }
    var targetPath = ouputDir ? ouputDir : Path.join(Path.parse(source).dir, 'generated');
