...
export default class WebpackBuilderService extends BaseBuilderService {
  ...
  */
  ...
  _buildWithConfigs(configs) {
    var self = this;
    this.builder.build(configs, configs.source, configs.output).then((resultPath) => {
      console.log('Build success to: ' + resultPath);
      self.sendBuildSuccessNotify([resultPath]);
    }).catch((err) => {
  ...
  sendBuildSuccessNotify(logObj) {
    // 1. Notify build success widget.
    BuilderNotifier.notifyBuildSuccess(logObj);
    // 2. Notify app via later build.js changes.
    // 3. Write build log to console-panel#server.
    BuilderNotifier.writeBuildSuccessLog(logObj);
  }

  sendBuildFailedNotify(err) {
    // 1. Notify build fail widget.
    BuilderNotifier.notifyBuildFailed(err);
    // 2. Notify app the error.
    ...
    // 3. Write build log to console-panel#server.
    BuilderNotifier.writeBuildFailedLog(err);
  }
